        ...
        if tag == "a":
            for i in attrs:
                if "myanimelist.net/character" in i[1]:
                    ...
                    if m is None:
                        return
                    if m.groups()[0] not in self.id_list:
...
def get_top_anime_ids(amount, offset, type="anime"):
    x = requests.get(
        f"https://api.myanimelist.net/v2/{type}/ranking?ranking_type=all&limit={amount}&offset={offset}",
        headers=HEADERS,
    )
        ...
        if favorites != "0":
            ...
            try:
                char_request = requests.get(
                    f" https://api.myanimelist.net/v2/characters/{character_id}?fields=first_name,last_name,pictures",
                    headers=HEADERS,
                )
                ...
                if "pictures" in char_json:
                    ...
                    if type == "anime":
                        new_character = Character(
                            id=character_id,
                            first_name=char_json["first_name"],
                            last_name=char_json["last_name"],
                            anime=anime.title,
                            manga="",
                            game=game_override,
                            images=pictures,
...
def save_to_db(database, character_list: list[Character], type="anime") -> None:
    ...
    for character in character_list:
        if type == "anime":
            cursor.execute(
                "SELECT anime_list FROM CHARACTERS WHERE ID = %(id)s",
                {"id": character.id},
            )
            old_anime_list = cursor.fetchone()
            if old_anime_list is not None and character.anime not in old_anime_list[0]:
                new_anime_list = old_anime_list[0] + "," + character.anime
                cursor.execute(
                    "UPDATE CHARACTERS SET anime_list = %(newlist)s WHERE ID = %(id)s",
                    {"id": character.id, "newlist": new_anime_list},
                )
            ...
            cursor.execute(
                sql,
                {
                    ...
                    "images": character.get_images_str(),
        ...
        if type == "manga":
            cursor.execute(
                "SELECT manga_list FROM CHARACTERS WHERE ID = %(id)s",
                {"id": character.id},
            )
            old_manga_list = cursor.fetchone()
            if old_manga_list is not None and character.manga not in old_manga_list[0]:
                new_manga_list = old_manga_list[0] + "," + character.manga
                cursor.execute(
                    "UPDATE CHARACTERS SET manga_list = %(newlist)s WHERE ID = %(id)s",
                    {"id": character.id, "newlist": new_manga_list},
                )
            sql = """INSERT INTO CHARACTERS VALUES (%(id)s,%(first_name)s,%(last_name)s,%(anime)s,%(images)s,%(value)s,%(manga)s,%(games)s) ON CONFLICT DO NOTHING"""
            cursor.execute(
                sql,
        ...
        if type == "game":
            cursor.execute(
                "SELECT games_list FROM CHARACTERS WHERE ID = %(id)s",
                {"id": character.id},
            )
            sql = """INSERT INTO CHARACTERS VALUES (%(id)s,%(first_name)s,%(last_name)s,%(anime)s,%(images)s,%(value)s,%(manga)s,%(games)s) ON CONFLICT DO NOTHING"""
            cursor.execute(
